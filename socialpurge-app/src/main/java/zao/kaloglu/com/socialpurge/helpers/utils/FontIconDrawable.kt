...
class FontIconDrawable @JvmOverloads constructor(
    ...
    */
    ...
    init {
        var iconstr = icon
        if (color == -1)
            this.color = R.attr.material_drawer_primary_text
        else
            this.color = ContextCompat.getColor(context, color)

        prepare()
        zao.kaloglu.com.socialpurge.components.CustomTypeFace.getTypeFace(context, zao.kaloglu.com.socialpurge.Sealed.Enums.FontType.FONT_ICON)?.let { typeface(it) }
        if (isResourceName)
